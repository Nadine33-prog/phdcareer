// ═══ 学术职位门槛评估 · 多维加权评分算法 ═══
import type { ThresholdForm, ThresholdResult, ScoreBreakdown, RadarDataPoint, ComparisonRow, CompareStatus, TargetTier } from "@/types/threshold";

// ── 院校层次分值 ──
const SCHOOL_SCORE: Record<string, number> = { "C9": 100, "海外名校": 95, "985": 82, "海外": 78, "211": 62, "省属": 40, "普通本科": 28, "民办": 12, "其他": 25, "无": 0 };
const SCHOOL_ORDER: Record<string, number> = { "C9": 5, "海外名校": 5, "985": 4, "海外": 3, "211": 3, "省属": 2, "普通本科": 1, "民办": 1, "其他": 1, "无": 0 };

// ── SCI 分区系数 ──
const Q_FACTOR: Record<string, number> = { "Q1": 1.0, "Q2": 0.7, "Q3": 0.45, "Q4": 0.25, "无": 0 };

// ── 海外经历类型系数 ──
const OVERSEAS_SCORE: Record<string, number> = { "无": 0, "短期访学(≤1年)": 30, "长期联培(>1年)": 55, "海外学位": 92 };

// ── 英语水平分值 ──
const ENGLISH_SCORE: Record<string, number> = { "无": 0, "CET-6": 25, "IELTS 6.5+": 55, "TOEFL 90+": 55 };

// ── 获奖分值 ──
const AWARD_SCORE: Record<string, number> = { "国家级": 100, "省部级": 55, "校级": 20, "无": 0 };

// ── 人才称号分值 ──
const TALENT_SCORE: Record<string, number> = { "国家级(青千/优青等)": 100, "省部级": 55, "无": 0 };

// ── 专著分值 ──
const MONOGRAPH_SCORE: Record<string, number> = { "独著1+": 60, "合著1+": 30, "0": 0 };

// ── 各层次高校门槛基准 ──
const TIER_BENCHMARK: Record<string, { edu: number; paper: number; grant: number; overseas: number; age: number; impact: number; award: number }> = {
...
export function calcEduScore(form: ThresholdForm): number {
  const s = SCHOOL_SCORE;
  let score = (s[form.phdSchool] || 50) * 0.50 + (s[form.masterSchool] || 50) * 0.25 + (s[form.bachelorSchool] || 50) * 0.15;
  if (form.hasPostdoc === "有") score += (s[form.postdocTier] || 50) * 0.10;
  else score += 50 * 0.10; // 无博后给基准分
  return Math.round(score);
}

/** 论文产出：SCI 分区加权 + CSSCI + EI */
export function calcPaperScore(form: ThresholdForm): number {
...
export function calcImpactScore(form: ThresholdForm): number {
...
/** 基金课题 */
export function calcGrantScore(form: ThresholdForm): number {
...
/** 海外经历 + 英语 */
export function calcOverseasScore(form: ThresholdForm): number {
...
/** 获奖成果 */
export function calcAwardScore(form: ThresholdForm): number {
...
export function calcPercentile(userScore: number): number { return clamp(100 - userScore, 1, 99); }

/** 计入常模的最低人数，达到后可把前台切到经验百分位 */
export const NORM_READY_TOTAL = 200;
/** 按学科 / 届次单独出分位时，该组至少要这么多人 */
export const NORM_GROUP_MIN = 50;
/** 同一人 / 同一浏览器 30 天内重填，只保留最新一条计入常模 */
export const NORM_DEDUPE_DAYS = 30;

/**
 * 经验百分位，含义与示意分位相同：大约位于前 X%。
 * below / N 是「超过了百分之多少」；这里再转成「前 X%」，方便以后直接替换 calcPercentile。
 */
export function calcEmpiricalPercentile(userScore: number, peerScores: number[]): number {
  if (peerScores.length === 0) return calcPercentile(userScore);
  const below = peerScores.filter((s) => s < userScore).length;
  const exceedPct = Math.round((below / peerScores.length) * 100);
  return clamp(100 - exceedPct, 1, 99);
}
...
export function judgeSchool(have: string, targetTier: string): CompareStatus {
  ...
  let need: number;
  if (targetTier.startsWith("顶尖")) need = 5;
  else if (targetTier.startsWith("985")) need = 4;
  else if (targetTier.startsWith("211")) need = 3;
  else need = 2;
  if (got >= need) return "pass";
  if (got >= need - 1) return "warn";
  return "fail";
}
...
export function evaluate(form: ThresholdForm): ThresholdResult {
  ...
  const userScore = Math.round(
    eduScore * 0.18 + paperScore * 0.28 + grantScore * 0.15 +
  ...
  if (hasTarget) {
    ...
    const sciF = parseInt(form.sciFirst) || 0, cssciF = parseInt(form.cssciFirst) || 0;
    ...
    targetRows = [
      ...
      { item: "CSSCI 论文（一作）", need: form.targetTier.startsWith("顶尖") ? "≥ 3 篇" : "≥ 2 篇", you: `${cssciF} 篇`, status: cssciF >= (form.targetTier.startsWith("顶尖") ? 3 : 2) ? "pass" : (cssciF >= 1 ? "warn" : "fail") },
    ...
    else if (matchPct >= 55) verdict = "部分达标";
    else verdict = "差距较大";
  }
...
function generateRecommendations(s: ScoreBreakdown, hasTarget: boolean, targetRows: ComparisonRow[] | undefined, form: ThresholdForm): string[] {
  const r: string[] = [];
  if (s.paperScore < 45) r.push("「论文产出」是核心短板。建议集中冲击 SCI/SSCI 一作 1–2 篇，优先投 Q1/Q2 期刊以提升竞争力。");
  else if (s.paperScore < 65) r.push("「论文产出」有提升空间。若能将现有 Q3/Q4 投稿升级为 Q2 以上，竞争力将显著增强。");
  if (s.grantScore < 40) r.push("「基金课题」较弱。重点冲刺国自然青年 / 国社科青年项目，这是青年学者最重要的入场券。");
  if (s.overseasScore < 35) r.push("「海外经历」不足。建议申请 CSC 公派访学或博士后海外联培项目，至少积累 1 年以上海外经历。");
  if (s.impactScore < 30) r.push("「学术影响力」偏低。h-index 需要时间积累，但可通过担任期刊审稿人、积极参与学术会议来加速建立学术声誉。");
  if (s.awardScore < 25) r.push("「获奖成果」偏弱。可关注省部级科研成果奖申报，发明专利和学术专著也能有效补充。");
  if (s.eduScore > 80) r.push("「教育背景」是你的明显优势，建议在求职材料中突出展示。");
  if (s.userScore > 80) r.push("综合竞争力位于同届前列，可冲刺更高层次的目标院系。");
  if (hasTarget && targetRows) {
    const failN = targetRows.filter(r => r.status === "fail").length;
    if (failN === 0) r.push("目标院系门槛已全部达标！建议同时投递同层次 2–3 个院系作为备选。");
    else if (failN >= 3) r.push(`短期内补齐多项硬指标有难度，建议下沉一档目标层次，或先入职积累成果 2–3 年后再冲刺。`);
  }
  return r;
}
