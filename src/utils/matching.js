...
export function computeMatch(answersB, answersC) {
    const bKeys = Object.keys(answersB), cKeys = Object.keys(answersC);
    if (bKeys.length === 0 && cKeys.length === 0)
        return [];
    ...
    for (const w of SIMPLIFIED_WEIGHTS) {
        wMap.set(`${w.itemId}_${w.categoryId}`, w.weight);
    }
    const raws = [];
    for (let d = 1; d <= 8; d++) {
        let interestNum = 0, interestDen = 0, skillNum = 0, skillDen = 0;
        for (const k of bKeys) {
            const w = wMap.get(`${k}_${d}`) || 0;
            if (w > 0) {
                interestNum += ((answersB[k] || 0) - meanB) * w;
                interestDen += w;
            }
        }
...
export function computeRadar(answersC) {
    const c = (ids) => { const vals = ids.map(i => answersC[i] || 0).filter(v => v > 0); return vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 0; };
...
export function checkConflicts(answersE, matches) {
    const rules = [
...
export function generateValuesQuestions(top5Ids) {
    const map = {
...
export function getTop1Label(matches) {
    if (matches.length === 0)
        return "数据不足以生成画像";
    const top = matches[0];
    const labels = { 1: "学术深耕者", 2: "公共服务者", 3: "政策洞察者", 4: "商业创新者", 5: "文化传播者", 6: "健康守护者", 7: "纪律执行者", 8: "多元探索者" };
    return labels[top.categoryId] || "多元探索者";
}
export function evaluateAssessment(answersB, answersC, answersD, answersE) {
