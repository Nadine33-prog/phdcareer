...
export const DEFAULT_RISK_RULES: RiskRule[] = [
  { id: 1, type: "预聘制 (Tenure-Track)", level: "高", scoreWeight: 25, patterns: ["预聘", "特聘", "tenure.?track", "3\\s*[+＋]\\s*3"], description: "该 JD 含预聘制条款，聘期内有明确的考核要求，未通过可能不予续聘。", interviewQuestions: ["首聘期考核如未通过，是否有过渡岗位或转长聘的可能？", "续聘考核的具体指标是什么？是否有申诉机制？"] },
  ...
  { id: 3, type: "年龄门槛", level: "中", scoreWeight: 20, patterns: ["≤\\s*35\\s*岁", "35\\s*周岁以下", "不超过\\s*35"], description: "该 JD 设定了年龄限制，对博士在读年限较长者不友好。", interviewQuestions: ["年龄限制是否有弹性空间（如对海外博后经历放宽）？"] },
...
export function scanJD(jdText: string, rules: RiskRule[] = DEFAULT_RISK_RULES): WarningResult {
  const detected: DetectedRisk[] = []; let totalScore = 0;
  for (const rule of rules) {
    const matched = rule.patterns.some(pattern => { try { return new RegExp(pattern, "i").test(jdText); } catch { return jdText.includes(pattern); } });
    if (matched) { totalScore += rule.scoreWeight; detected.push({ type: rule.type, level: rule.level, desc: rule.description }); }
  }
